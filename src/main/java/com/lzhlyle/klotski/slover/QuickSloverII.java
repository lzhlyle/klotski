...
//        System.out.println(step + ": " + queue.size() + ": " + visited.size());
    ...
    private void output(Map<int[], int[]> paths, int[] curr) {
        ...
        this.printLayout(curr);

        int count = 0;
        while (!stack.isEmpty()) {
            System.out.println("第 " + (++count) + " 步");
            this.printLayout(stack.pop());
        }
    }

    private void printLayout(int[] curr) {
        int occupying = 0b0;
        for (int b : curr) occupying |= b;
    ...
    public static void main(String[] args) {
        QuickSloverII slover = new QuickSloverII();
        duration = new Duration();
        slover.minSteps(slover.standard);
